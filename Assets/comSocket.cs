...
public class comSocket : MonoBehaviour {
	...
	float m_zCoef;
	Socket m_clientS;
	public bool m_lampState = false;
	bool m_lastLampState = false;
	...
	void Start () 
	{
		...
		//m_clientS = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
		//m_clientS.Connect ("127.0.0.1", 80);
	...
	void Update () 
	{
		...
		if (l_speed > 0.0f) 
		{
			l_speed = (l_speed - 8.0f) * 6.0f;//2.5f;
		} 
		if( l_speed > 100.0f ) 
			l_speed = 100.0f;
		if (l_speed < 0.0f)
		...
		// lamp
		//
		if (m_lampState != m_lastLampState) 
		{
			//frame type
			m_buffer [0] = 0x0D;
			//size
			...
			m_buffer [3] = 0x00;
			m_buffer [4] = 0x00;
			m_buffer [5] = (m_lampState == true) ? (byte)1 : (byte)0;
			try 
			{
				m_stream.Write (m_buffer, 0, 6);
			} 
